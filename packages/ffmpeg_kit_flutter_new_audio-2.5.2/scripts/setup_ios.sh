#!/bin/bash
#
# Installs the bundled FFmpegKit iOS frameworks into
...
#     successful extraction + conversion + sanity check. A failure
...
VERSION="8.1.2"
VARIANT="audio"
ARCHIVE_NAME="ffmpeg-kit-ios-${VARIANT}-${VERSION}.zip"
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
BUNDLED_ARCHIVE="${SCRIPT_DIR}/../ios/prebuilt/${ARCHIVE_NAME}"
BUNDLED_SHA256="ca61adb8720a09097189c3e0d3982ee11b7a89fa9f202c197f33d1cde166a915"
...
if [ -d "Frameworks/ffmpegkit.xcframework" ]; then
  echo "[ffmpeg_kit_flutter] iOS frameworks already present — skipping setup."
  exit 0
fi
...
fail() {
  {
    ...
    echo "Restore the plugin's ios/prebuilt directory, then run pod install again."
    echo "======================================================================"
  } >&2
  exit 1
}
...
echo "[ffmpeg_kit_flutter] Preparing bundled iOS frameworks ($VARIANT $VERSION)..."
