...
# from the bundled ffmpeg-kit release zips, zip them for use as SPM binary
# targets, and emit their checksums.
#
# Usage: build_spm_artifacts.sh <variant> <version>
#   e.g. build_spm_artifacts.sh full-gpl 8.1.2
#
# Inputs:  ios/prebuilt and macos/prebuilt archives in this local package.
...
IOS_ARCHIVE="${SCRIPT_DIR}/../ios/prebuilt/ffmpeg-kit-ios-${VARIANT}-${VERSION}.zip"
MACOS_ARCHIVE="${SCRIPT_DIR}/../macos/prebuilt/ffmpeg-kit-macos-${VARIANT}-${VERSION}.zip"
...
mkdir -p "$WORK"
cp "$IOS_ARCHIVE" "$WORK/ios.zip"
cp "$MACOS_ARCHIVE" "$WORK/macos.zip"
cd "$WORK"
